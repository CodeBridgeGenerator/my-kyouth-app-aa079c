...
import addtocartFakerFactory from "./addtocartFakerFactory";
...
const AddtocartFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = addtocartFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("addtocart").create(elem));
...
export default connect(mapState, mapDispatch)(AddtocartFakerDialogComponent);
